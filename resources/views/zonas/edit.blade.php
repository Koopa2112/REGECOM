...
<div class="container py-4">
    ...
    <form action="{{ url('zonas/' .$zona->id ) }}" method="post" id="actualizarZona">
        @method("PUT")
        @csrf
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-5">
                <input type="text" class="form-control" name="nombre_zona" id="nombre_zona" value="{{$zona->nombre_zona}}"required>
            </div>
        </div>
        ...
        function update(){
            ...
            var confirmacion = confirm('¿Estás seguro de que deseas actualizar la información de la zona?');

            // Si el usuario hace clic en "Aceptar", se enviará el formulario
            if (confirmacion) {
                document.getElementById('actualizarZona').submit();
            }
