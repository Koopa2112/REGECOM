        ...
        <form action="{{ url('ventas/' .$venta. '/saveDate') }}" method="post" id="registrarFecha">
            ...
            <div class="mb-3 row">
                ...
                <select name="fecha_entrega" class="form-select" id="fecha_entrega" required>
                    ...
                    <option value="{{ $fecha_entrega->id }}">{{ date("d M Y", strtotime($fecha_entrega->fecha_entrega)) }}</option>
                    @endforeach
                </select>
                </div>
            </div>
                ...
                <button type="button" class="btn btn-danger" id="btnCancelar">CANCELAR VENTA</button>
                    ...
                    <script>
                        document.getElementById('btnCancelar').addEventListener('click', function() {
                            // Muestra un cuadro de diálogo de confirmación
                            var confirmacion = confirm('¿Estás seguro de que deseas cancelar la venta? No podrás deshacer esta acción');
